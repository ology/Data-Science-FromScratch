...
$got = $ds->logistic_prime(-10);
is sprintf('%.4f', $got), '0.0000', 'logistic_prime';
$got = $ds->logistic_prime(0);
is sprintf('%.4f', $got), '0.2500', 'logistic_prime';
$got = $ds->logistic_prime(10);
is sprintf('%.4f', $got), '0.0000', 'logistic_prime';

SKIP: {
skip 'Incorrect results! What the?', 5;
...
my @ys = map { $_->[2] } @data;
my @rescaled_xs = $ds->rescale(@xs);
my ($x_train, $x_test, $y_train, $y_test) = $ds->train_test_split(\@rescaled_xs, \@ys, 0.33);
my $beta = [ map { rand } @{ $xs[0] } ];
my $loss = 0;
for my $i (1 .. 5000) {
    my $gradient = $ds->negative_log_gradient($x_train, $y_train, $beta);
    $beta = $ds->gradient_step($beta, $gradient, -0.01);
    $loss = $ds->negative_log_likelihood($x_train, $y_train, $beta);
}
is_deeply $beta, [-2.0239, 4.6930, -4.4698], 'beta';
is sprintf('%.4f', $loss), '39.963', 'loss';
...
is_deeply $beta, [8.9, 1.6, 0.000288], 'unscaled';

$beta = [8.9, 1.6, 0.000288];

my ($true_pos, $false_pos, $tru_neg, $false_neg) = (0, 0, 0, 0);

for my $i (0 .. @$x_test - 1) {
    my $prediction = $ds->logistic($ds->vector_dot($beta, $x_test->[$i]));
