...
is $ml->entropy([1]), 0, 'entropy';
is $ml->entropy([0.5, 0.5]), 1, 'entropy';
my $got = $ml->entropy([0.25, 0.75]);
is sprintf('%.4f', $got), '0.8113', 'entropy';

$got = $ml->class_probablities(['a','b']);
is_deeply $got, [0.5, 0.5], 'class_probablities';

is $ml->data_entropy(['a']), 0, 'data_entropy';
is $ml->data_entropy([0, 1]), 1, 'data_entropy';
is $ml->data_entropy([1,2,3,4]), 2, 'data_entropy';
...
my %h = $ml->partition_by(\@inputs, 'level');
is_deeply $h{Senior}[0], $expect, 'partition_by';

$got = $ml->partition_entropy_by(\@inputs, 'level', 'did_well');
is sprintf('%.4f', $got), '0.6935', 'partition_entropy_by';
$got = $ml->partition_entropy_by(\@inputs, 'lang', 'did_well');
is sprintf('%.4f', $got), '0.8601', 'partition_entropy_by';
$got = $ml->partition_entropy_by(\@inputs, 'tweets', 'did_well');
is sprintf('%.4f', $got), '0.7885', 'partition_entropy_by';
$got = $ml->partition_entropy_by(\@inputs, 'phd', 'did_well');
is sprintf('%.4f', $got), '0.8922', 'partition_entropy_by';

my @senior_inputs = map { $_ } grep { $_->{level} eq 'Senior' } @inputs;

$got = $ml->partition_entropy_by(\@senior_inputs, 'lang', 'did_well');
is sprintf('%.4f', $got), '0.4000', 'partition_entropy_by';
$got = $ml->partition_entropy_by(\@senior_inputs, 'tweets', 'did_well');
is sprintf('%.4f', $got), '0.0000', 'partition_entropy_by';
$got = $ml->partition_entropy_by(\@senior_inputs, 'phd', 'did_well');
...
my $tree = $ml->build_tree_id3(\@inputs, ['level', 'lang', 'tweets', 'phd'], 'did_well');
is_deeply $tree, $expect, 'build_tree_id3';

ok $ml->classify($tree, {level => 'Junior', lang => 'Java', tweets => 1, phd => 0}), 'classify';
ok ! $ml->classify($tree, {level => 'Junior', lang => 'Java', tweets => 1, phd => 1}), 'classify';
ok $ml->classify($tree, {level => 'Intern', lang => 'Java', tweets => 1, phd => 1}), 'classify';
